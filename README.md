...
It is more secure to create different `tokens` when a user make a login request from different `IP Addresses`. This can be done by adding an `ip_address` key-value in the `payload`. This requires to update the `CustomRequest` interface (`./src/libs/types.ts`)
